...
CreateJointVelocitiesCallback(
  ...
  return [&global_handler, &state_publisher, &model, &current_state_info,
    ...
    if (!global_handler->running) {
      franka::JointVelocities zero_velocities{
    ...
    global_handler->traj_interpolator_ptr->GetNextStep(global_handler->time,
                                                       desired_q);
    std::cout << "q\n" << desired_q.transpose() << std::endl;

    Eigen::Matrix<double, 7, 1> desired_qv =
        desired_qv - current_state_info->joint_positions;
    std::cout << "desired_qv\n" << desired_qv << std::endl;
    ...
    joint_velocities =
        global_handler->controller_ptr->Step(robot_state, desired_qv);
